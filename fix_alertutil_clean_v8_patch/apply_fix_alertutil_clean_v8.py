...
ROOT = Path.cwd()
target = ROOT / "client/src/main/java/com/auction/client/util/AlertUtil.java"

if not target.exists():
    raise FileNotFoundError(f"Không tìm thấy file: {target}")

backup_dir = ROOT / f"backup_fix_alertutil_clean_v8_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
backup_dir.mkdir(parents=True, exist_ok=True)
backup_file = backup_dir / "AlertUtil.java"
backup_file.write_text(target.read_text(encoding="utf-8-sig"), encoding="utf-8")

clean_code = """package com.auction.client.util;
...
print("DONE: AlertUtil.java đã được thay bằng bản sạch.")
