...
public final class AlertUtil {
    ...
    public static void showStyledNativeAlert(Alert.AlertType type, String title, String message) {
        Alert alert = new Alert(type);
        alert.setTitle(normalizeTitle(title, type));
        alert.setHeaderText(normalizeTitle(title, type));
        alert.setContentText(normalizeMessage(message));
        try {
            alert.initStyle(StageStyle.TRANSPARENT);
        } catch (IllegalStateException ignored) {
            // Dialog may already be initialized in rare cases; styling still applies below.
        }
        alert.setOnShown(event -> styleDialog(alert));
        styleDialog(alert);
        alert.showAndWait();
    }
