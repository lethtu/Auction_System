...
public class SidebarController {
    ...
    private void ensureExpandedSidebarChrome() {
        if (sidebarContainer != null && !isSidebarCollapsed) {
            sidebarContainer.setMinWidth(214);
            sidebarContainer.setPrefWidth(214);
            sidebarContainer.setMaxWidth(214);
        }
        if (sidebarContent != null && !isSidebarCollapsed) {
            sidebarContent.setPadding(new Insets(28, 10, 28, 10));
            sidebarContent.setAlignment(Pos.TOP_LEFT);
            sidebarContent.setStyle("-fx-background-color: rgba(255,255,255,0.97); -fx-background-radius: 30; -fx-border-color: #f2a6d8; -fx-border-radius: 30; -fx-border-width: 1.8; -fx-effect: dropshadow(gaussian, rgba(224,64,160,0.10), 18, 0.12, 0, 6);");
        }
    }
    ...
    public void toggleSidebar() {
        ...
        if (isSidebarCollapsed) {
            ...
            sidebarContainer.setMinWidth(76);
            sidebarContainer.setPrefWidth(76);
            sidebarContainer.setMaxWidth(76);
            sidebarContent.setPadding(new Insets(28, 0, 28, 0));
            ...
            for (javafx.scene.Node node : sidebarContent.getChildren()) {
                if (node instanceof Button) {
                    ...
                    if (tooltipText != null && !tooltipText.isEmpty()) {
                        ...
                        tooltip.setStyle(
                                "-fx-background-color: #e040a0; -fx-text-fill: white; -fx-font-weight: 800; -fx-background-radius: 8px; -fx-padding: 6px 12px; -fx-font-size: 13px;");
                    ...
                    if (btn == btnStartSelling) {
                        btn.setStyle(
                                "-fx-font-family: 'DM Sans'; -fx-font-size: 15px; -fx-background-color: #e040a0; -fx-text-fill: white; -fx-font-weight: 800; -fx-background-radius: 24px; -fx-padding: 0px; -fx-cursor: hand; -fx-effect: dropshadow(three-pass-box, rgba(224, 64, 160, 0.3), 16, 0, 0, 4);");
                    }
    ...
    private void applyExpandedSidebar() {
        ...
        for (javafx.scene.Node node : sidebarContent.getChildren()) {
            if (node instanceof Button) {
                ...
                btn.setMinWidth(184);
                btn.setMaxWidth(184);
                btn.setPrefHeight(btn == btnStartSelling ? 46 : 46);
                btn.setMinHeight(btn == btnStartSelling ? 46 : 46);
                btn.setMaxHeight(btn == btnStartSelling ? 46 : 46);
                btn.setAlignment(Pos.CENTER_LEFT);
                if (btn == btnStartSelling) {
                    btn.setStyle(
                            "-fx-font-family: 'DM Sans'; -fx-font-size: 15px; -fx-background-color: #e040a0; -fx-text-fill: white; -fx-font-weight: 800; -fx-background-radius: 23px; -fx-padding: 0 14px; -fx-cursor: hand; -fx-effect: dropshadow(three-pass-box, rgba(224, 64, 160, 0.3), 16, 0, 0, 4);");
                }
            } else if (node instanceof Label) {
    ...
    private void applySidebarButtonStyle(Button button, boolean active) {
        ...
        String padding = isSidebarCollapsed ? "0px" : "10px 18px";
        String radius = isSidebarCollapsed ? "24px" : "23px";

        button.setStyle("-fx-font-family: 'DM Sans'; -fx-font-size: 15px; -fx-background-color: "
                + backgroundColor
                + "; -fx-text-fill: "
                + textColor
                + "; -fx-font-weight: 800; -fx-background-radius: " + radius + "; -fx-padding: " + padding
                + "; -fx-cursor: hand;");
        if (!isSidebarCollapsed) {
        ...
        if (button.getGraphic() instanceof Label) {
            ((Label) button.getGraphic()).setStyle(
                    "-fx-font-family: 'Material Symbols Outlined'; -fx-font-size: 21px; -fx-font-weight: normal; -fx-text-fill: "
                            + textColor
                            + ";");
        }
    }
